      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="max-h-[70vh] space-y-6 overflow-y-auto p-4">
            ...
            {/* Go To section */}
            <ShortcutSection title="GO TO" shortcuts={SHORTCUT_DEFINITIONS.goTo} color="orange" />
          </div>
          ...
          <div className="flex items-center gap-2 border-t border-neon-cyan/30 px-4 py-2 text-xs text-muted-foreground">
            ...
            <span>
              Press <kbd className="border border-neon-cyan/50 px-1 text-neon-cyan">ESC</kbd> to
              close
            </span>
          </div>
        </div>
...
function ShortcutSection({ title, shortcuts, color }: ShortcutSectionProps) {
  ...
  return (
    <div>
      <h3 className={cn('mb-2 text-xs font-bold', colors.title)}>{title}</h3>
      <div className={cn('space-y-2 border-l-2 pl-3', colors.line)}>
