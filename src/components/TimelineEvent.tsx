...
export function TimelineEvent({
  ...
  return (
    <div className="absolute top-1/2 z-10 -translate-y-1/2" style={{ left: `${position * 100}%` }}>
      {/* Event dot */}
      <button
        onClick={onClick}
      ...
      >
        ...
        {event.isNew && (
          <div
            className={cn('absolute inset-0 animate-ping rounded-full', colors.bg, 'opacity-75')}
          />
        )}
      </button>
      ...
      {showTooltip && (
        <div
          ...
          style={
            {
              '--glow-color': colors.bg.includes('cyan')
                ? 'hsl(180 100% 50%)'
                : colors.bg.includes('magenta')
                  ? 'hsl(300 100% 50%)'
        ...
        >
          ...
          <div className="space-y-1">
            <div className={cn('truncate font-bold uppercase', colors.bg.replace('bg-', 'text-'))}>
              {event.event_type?.split('.').pop() || 'event'}
            </div>
            <div className="truncate text-muted-foreground">{event.title}</div>
            <div className="flex items-center gap-2 text-muted-foreground/70">
              <span>{formatTime(event.created_at)}</span>
              <span className={cn('border px-1', colors.border)}>
...
export function TimelineEventCluster({
  ...
  const dominantCategoryId = Object.entries(categoryCounts).sort(([, a], [, b]) => b - a)[0][0];
